...
3.5 **강의 틀(기본)** — 제목 "이런 ○○, 이제 클로드 코드로 직접 만드세요" · 인트로는 시청자의 문제·원하는 결과로 열고 완성작을 **유형으로**(우리 앱·회사 이름으로 소개하지 않음 — 시청자는 모른다) · 30초 개념 · 방법 ① 가이드로 ② 레퍼런스 영상으로 · 유형별 예시 3개 · 가이드 파일(설명란 링크) · 챕터 10개 안팎
...
6.5 **스토리보드 먼저** — 전체 렌더 전에 핵심 장면 정지 이미지(`--still --query f=N`)를 한 장으로 모아 보여 주고 확인받는다. 레퍼런스 영상이 있으면 프레임·컷 시점을 뽑아 스타일 명세(템포·컷 길이·글꼴·색·전환)로 정리해 템플릿 값에 옮긴다
...
8. **배포 문구** — `publish.md` 하나에 순서대로: 유튜브(제목 후보 3·설명·챕터 타임스탬프·태그·GitHub/노션 링크) → 쇼츠/릴스 캡션·해시태그 → 캐러셀 장별 문구 → 쓰레드 연재(3~4개: 첫 줄은 놀라운 주장·숫자로 후킹, 1인칭 반말, 한 글에 하나. "여러분"·"~하시나요?" 같은 호칭·말 걸기 금지 — 오글거림). **롱폼 대본이 원본**이고 나머지는 거기서 잘라 쓴다.
9. **공유**
...
- **제목·썸네일·첫 문구는 "[모델·도구]로 [끝내주는] [결과물] 제작" 형식** — 예: "오푸스 5.5로 끝내주는 모션그래픽 광고영상 제작", "클로드 코드로 영상 만들기". 우리 앱·회사 이름은 제목에 넣지 않는다. "영상 AI 없이"처럼 설명이 필요한 부정형·대비형은 쓰지 않는다 (001에서 헷갈린다는 피드백).
